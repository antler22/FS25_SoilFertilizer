...
function SoilHUD:drawPanel()
    ...
    -- Shows "(lb/ac)" when useImperialUnits is true, otherwise "(ppm)".
    cy = cy - pad * 0.8
...
function SoilHUD:drawNutrientRow(label, nutrient, px, cy, pw, s, fontMult, info, profile, fillType)
    ...
    -- Conversion: internal scale → ppm via PPM_DISPLAY multiplier;
    --             ppm → lb/ac via PPM_TO_LB_PER_AC (1 ppm ≈ 2 lb/ac at 6-7" depth).
    ...
    local dispMult  = imperial
        and ppmMult * (ppm.PPM_TO_LB_PER_AC or 2.0)
        or  ppmMult
